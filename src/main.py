...
def file_dialog_command():
	...
	if(tracefile_name != ""):
		...
		if (len(TrameList.get_liste()) == 0):
			...
			# Display Frames, and Store Frames to be Exported
...
# Sort Frames to get chronological order
...
def handle_filters(e):
  ...
  # Filters Handling
	if(filtre.replace(" ", "") == ""):
		...
		# The frames must be displayed and exported in chronological order
...
# Text Buttons used to insert text related to the protocol selected
def toggle_all():
	for widget in content_text.winfo_children():
		widget.destroy()
...
def button_display():
	...
	# Display button according to the protocols within the frame
	# All
	...
	if (trame.get_ethernet() != None):
		...
		if (trame.get_ip() != None):
			...
			if (trame.get_transport() != None):
				...
				# HTTP within TCP segment or UDP datagram
				if (trame.get_http() != None):
...
# Linking Listbox and Text Widgets
def on_selected(e):
	for widget in content_text.winfo_children():
		widget.destroy()
