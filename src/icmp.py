...
class Icmp:
	...
	def __init__(self, typ, frame):
		...
		if ((self.typ_icmp == "08" or self.typ_icmp == "00") and self.code == "00"):
			...
			self.id = frame[8:12]
			self.seq_num = frame[12:16]
			self.orig_timestamp = frame[16:24]
			self.rec_timestamp = frame[24:32]
			self.tran_timestamp = frame[32:40]
	...
	def get_orig_timestamp(self):
		return self.orig_timestamp

	def get_rec_timestamp(self):
		return self.rec_timestamp

	def get_tran_timestamp(self):
		return self.tran_timestamp
	...
	def __str__(self):
		...
		if ((self.typ_icmp == "08" or self.typ_icmp == "00") and self.code == "00"):
			...
			chaine += f"\n\tIdentifier: 0x{self.id}\
			\n\tSequence Number: 0x{self.seq_num}\
			\n\tOriginal Timestamp: 0x{self.orig_timestamp}\
			\n\tReceive Timestamp: 0x{self.orig_timestamp}\
			\n\tTransport Timestamp: 0x{self.tran_timestamp}"
